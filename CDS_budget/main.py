...
from .column_row_generation import Interdependent_Nested_CnCG as Model_CG
from .logging_setup import experiment_logger, experiment_log_path, BNB_status_logger, CG_status_logger
...
HEADER = (
    ...
    "N,K,b_a,b_p,structure_propagation,strategy_propagation,seed,TimeLimit,protectors,bridges,"
    ...
    "BNB.Error,"
    "CG.SLURM_ARRAY_TASK_ID,CG.Iters,CG.Time,CG.P.Time,CG.A.Time,CG.D.Time,"
    "CG.P.Cols,CG.P.Rows,CG.A.Calls,CG.D.Calls,"
    "CG.Weight,CG.Gap,"
    "CG.Opt.Policy,CG.Opt.Strategy,CG.Opt.Structure,"
    "CG.Error"
)
...
def log_dicts_csv(logger, log_path, graph_config, bnb_stats, cg_stats):
    ...
    merged = {
        ...
        **{f"CG.{k}":  v for k, v in cg_stats.items()},
    }
...
def generate_small_world_clique(n: int, clique_size: int, protectors:int, bridges:int, seed: int):
    ...
    num_bridges = bridges  # how many edges to add between each pair of cliques
    ...
    for i in G.nodes():
        ...
        to_protect = neighbors[:protectors] + [i]
...
def plot_colored_graph(G, protected_by, w_hat=[], x_hat=[], y_hat=[], name:str="test"):
    log_directory = os.getenv("EXPERIMENT_DIR", "CDS_budget/logs/local")
...
def run_BNB(n: int, k: int, b_p: int, b_a: int, strategy_propagation: int,
            structure_propagation: int, seed: int, time_limit: int = 60 * 60 * 3, protectors:int=2, bridges:int=4):
    
    G, protected_by, protectors_of = generate_small_world_clique(n=n,clique_size=k,protectors=protectors, bridges=bridges, seed=seed)
...
def run_CCG(n:int, k:int, b_p:int, b_a:int, seed:int, time_limit:int=60*60*3, protectors:int=2, bridges:int=4):
    ...
    result = model.solve_three_level()
    plot_colored_graph(G, protected_by, result[1], result[2], result[3], name="CCG")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n",                     type=int,   default=30,                    help="Number of nodes in the graph")
    parser.add_argument("--k",                     type=int,   default=5,                     help="Clique size")
    parser.add_argument("--b_p",                   type=int,   default=1,                     help="Protector budget")
    parser.add_argument("--b_a",                   type=int,   default=3,                     help="Attacker budget")
    parser.add_argument("--strategy_propagation",  type=int,   default=0, choices=[-1, 0],    help="Protector constraint propagation type")
    parser.add_argument("--structure_propagation", type=int,   default=1, choices=[-1, 0, 1], help="Attacker constraint propagation type")
    parser.add_argument("--seed",                  type=int,   default=1,                     help="Random seed for reproducibility")
    parser.add_argument("--time_limit",            type=float, default=60*60*3,               help="Time limit in seconds")
    parser.add_argument("--bnb_only",              action="store_true",                       help="Run only the BnB algorithm")
    parser.add_argument("--protectors",            type=int,   default=2,                     help="Number of protectors per node within a clique")
    parser.add_argument("--bridges",               type=int,   default=4,                     help="Maximum inter-clique bridges per pair of cliques")
    ...
    run_SLURM(
        ...
        seed=args.seed, time_limit=args.time_limit, bnb_only=args.bnb_only,
        protectors=args.protectors, bridges=args.bridges,
    )
