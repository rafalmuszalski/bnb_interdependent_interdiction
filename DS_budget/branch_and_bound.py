...
from .model import ThreeLevelGame, EarlyProtectorTermination, TimeOutTermination
...
class BranchAndBound:
    ...
    def print_status(self, node: Node):
        ...
        BNB_details_logger.info(
            ...
            f"Defender Weight: {node.objective_value}\n"
            ...
            f"N.Time  : {perf_counter() - self.start_time:>10.5f}\n"
            f"P.Time  : {self.model.time_solving_protector:>10.5f}\n"
            f"A.Time  : {self.model.time_solving_attacker:>10.5f}\n"
            f"A.Calls : {self.model.num_critical_strategies_added:>10}\n"
            f"D.Time  : {self.model.time_solving_defender:>10.5f}\n"
            f"D.Calls : {self.model.num_critical_structures_added:>10}\n\n"
        )
    ...
    def is_attacker_subopt(self, node: Node) -> bool:
        ...
        if new_obj > node.objective_value + self.epsilon:
    ...
    def run_heuristic(self, policy=[], ran_at_bnb_node: str = "*"):
        ...
        self.delayed_log_of_heuristic_solution = (
            ran_at_bnb_node, policy, new_attack, defender_structure, defender_weight, t_elapsed
        )
    ...
    def process_node(self, node:Node, time_limit:float):
        ...
        # Feasible Solution to the INTERDEPENDENT problem (integer solution)
            ...
            if node.objective_value < self.upper_bound: # New upper bound
                ...
                self.model.UB = node.objective_value
    ...
    def solveBNB(self):
        ...
        while self.node_queue and self.num_nodes < self.max_num_nodes:
            ...
            if node.objective_value <= self.lower_bound - self.epsilon:
                BNB_status_logger.info(
                    f"Objective value {node.objective_value} is less than lower bound {self.lower_bound}, Node: {node.name}"
                )
                raise ValueError("Node Objective Value is less than lower bound.")
